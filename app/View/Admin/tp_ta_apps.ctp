<?php $this->set("title_for_layout", "TP TA Applications") ?>
...
<table class="table table-bordered">
    ...
    <?php foreach($tp_ta_apps as $app): ?>
        <?php $id = $app['User']['id']; $appid = $app['TpTaApp']['id']; ?>
                <tr <?php if(!$app['TpTaApp']['selected']){echo 'class="tr-hide"';}?> >
                    <td>
                        <?php echo $app['User']['name']; ?> <br/>
                        <?php if($app['TpTaApp']['selected']) { echo '<span class="small-badge green">TA</span>';} ?>
                    </td>
                    <td>
                    ...
                    <td align='right'>
                        <?php 
                            ...
                            if($app['TpTaApp']['selected']) {
                                echo $this->Html->link('Remove TA', array('controller'=>'admin', 'action'=>'remove_tp_ta', $app['TpTaApp']['id']), array('class'=>'btn btn-xs btn-danger'), 
                                    'Are you sure to !!REMOVE!! '.$app['User']['name'].' as a TA?');
                            } else {
                                echo $this->Html->link('Select TA', array('controller'=>'admin', 'action'=>'add_tp_ta', $app['TpTaApp']['id']), array('class'=>'btn btn-xs btn-default'),
                                    'Are you sure to ||SELECT|| '.$app['User']['name'].' as a TA?');
                            }
                        ?>
                    </td>
                </tr>
