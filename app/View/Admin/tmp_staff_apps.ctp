        ...
        <tr <?php if(!$app['TmpStaffApp']['selected']){echo 'class="tr-hide"';}?> >
            ...
            <td align='right'>
                <?php 
                    ...
                    if($app['TmpStaffApp']['selected']) {
                        echo $this->Html->link('Remove Staff', array('controller'=>'admin', 'action'=>'remove_tmp_staff', $app['TmpStaffApp']['id']), array('class'=>'btn btn-xs btn-danger'), 
                            'Are you sure to !!REMOVE!! '.$app['User']['name'].' as a staff?');
                    } else {
                        echo $this->Html->link('Select Staff', array('controller'=>'admin', 'action'=>'add_tmp_staff', $app['TmpStaffApp']['id']), array('class'=>'btn btn-xs btn-default'),
                            'Are you sure to ||SELECT|| '.$app['User']['name'].' as a staff?');
                    }
                ?>
            </td>
        </tr>
